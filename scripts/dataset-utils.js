...
export async function discoverArchiveUrl(sourceConfig) {
  ...
  const candidates = Array.from(matches).filter((candidate) => {
    ...
    return /\.(zip|tar\.bz2|tar\.gz|tgz|npy)(?:$|\?)/i.test(candidate) || /\/download(?:$|\?)/i.test(candidate);
  });
...
export async function writeMetadata(entries) {
  ...
  await fs.writeFile(datasetSummaryPath, `${JSON.stringify(buildSummary(sorted), null, 2)}\n`, "utf8");
}
